        ...
        <div class="float-right">
            @can("$route.store")
                @if ($type == 'redirect')
                    ...
                    @endif
                    @if ($excel)
                        <button type="button" class="btn btn-sm btn-secondary mr-1" data-bs-toggle="modal"
                            data-bs-target="#modal-excel">Excel</button>
                    @endif
                @else
                    <button type="button" class="btn btn-sm btn-primary float-right" data-placement="left"
                        data-bs-toggle="modal" data-bs-target="#modal-{{ $modalName }}">
                        <i class="fas fa-plus"></i> {{ $message }}
                    </button>
                @endif
            @endcan
